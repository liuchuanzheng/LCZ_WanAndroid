package com.liuchuanzheng.lcz_wanandroid.widgets
...
class TagView(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : LinearLayout(context, attrs, defStyleAttr) {
    ...
    private fun itemView(text: String, color: Int, bg: Int): View {
        ...
        layoutParams.setMargins(0, 0, ResTool.getDimens(context,R.dimen.d_5), 0)
        textView.layoutParams = layoutParams
        textView.text = text
        textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ResTool.getDimens(context,R.dimen.d_12).toFloat())
        textView.setTextColor(ResTool.getColor(context,color))
        textView.setBackgroundResource(bg)
        textView.setPadding(ResTool.getDimens(context,R.dimen.d_5), ResTool.getDimens(context,R.dimen.d_1), ResTool.getDimens(context,R.dimen.d_5), ResTool.getDimens(context,R.dimen.d_3))
        return textView
    }
