...
import com.liuchuanzheng.lcz_wanandroid.widgets.glide.RoundedCornersTransformation
...
object GlideUtil {
    ...
    fun loadRoundedImage(context: Context, url: String?, dimenRes: Int, imageView: ImageView) {
        with(context).load(url).apply(getOptions())
            .transform(RoundedCorners(ResTool.getDimens(context,dimenRes))).into(imageView)
    }
    ...
    fun loadRoundedCornersImage(
        ...
        with(context).load(url).apply(getOptions()).transform(
            RoundedCornersTransformation(
                ResTool.getDimens(context, dimenRes),
                cornerType,
                scaleType
            )
        ).into(imageView)
    ...
    fun loadRoundedCornersStrokeImage(
        ...
        loadRoundedCornersStrokeImage(
            ...
            RoundedCornersTransformation.FIT_CENTER,
            true,
            ResTool.getDimens(context, R.dimen.d_1),
            ResTool.getColor(context, R.color.c_99),
            imageView
        )
    }
    ...
    fun loadCircleWithBorderImage(
        ...
        with(context).load(url).apply(getOptions()).transform(
            CircleWithBorder(
                ResTool.getDimens(context, strokeWidth ?: 2),
                ResTool.getColor(context, strokeColor ?: R.color.white)
            )
        ).into(imageView)
    }
